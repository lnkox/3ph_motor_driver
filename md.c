...
#define OVER_MOTORTEMP  PIND.2  // - Перегрев двигателя - нормальное состояние лог.0 при появлении лог.1. сделать стоп привода, подать на PC1 сигнал с частотой 1Гц. Разрешить работу только при появлении лог.0.
...
interrupt [TIM0_OVF] void timer0_ovf_isr(void)
{
     ...
     if (sinseg>=sinseg_period) 
     {
        ...
        if (mode==1 && amplitude>0 ) {gen_next_sinpos();}             
     }
    sys_timer_cnt++;
...
{
   float amp;
   if (freq==0) {amplitude=0;sinseg_period=124; return;}   
